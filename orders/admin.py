...
class NotificationAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "mark-all-read/",
                self.admin_site.admin_view(self.mark_all_read),
                name="notifications-mark-all-read",
            ),
            path(
                "mark-read/<int:pk>/",
                self.admin_site.admin_view(self.mark_read),
                name="notifications-mark-read",
            ),
            path(
                "unread/",
                self.admin_site.admin_view(self.get_unread_notifications),
    ...
    def get_unread_notifications(self, request):
        notifications = models.Notification.objects.filter(is_read=False).order_by(
            "-created_at"
        )[:20]
...
class OrderItemAdmin(admin.ModelAdmin):
    ...
    list_filter = ["order__restaurant", "applied_promotion"]
    list_per_page = 10
...
class CartItemAdmin(admin.ModelAdmin):
    list_display = ["id", "cart", "menu_item", "qty"]
    list_per_page = 10
    search_fields = ["cart__id", "menu_item__title"]
