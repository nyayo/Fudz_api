...
class ApiService extends GetxService {
      ...
      if (response.statusCode == 401 || response.statusCode == 403) {
        ...
        if (responseBody.contains('token') &&
          ...
          if (retryCount < 1) {
            ...
            if (refreshed) {
              print(
                '✅ Token refreshed, retrying request (attempt ${retryCount + 1})',
              );
  ...
  Future<dynamic> handleResponse(
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      print(
        'API ${response.statusCode} Unauthorized/Forbidden - Token might be expired',
      );
  ...
  Future<bool> refreshToken() async {
    ...
    try {
      ...
      if (response.statusCode == 401 || response.statusCode == 403) {
        _logDebug(
          'Refresh token rejected (${response.statusCode}), clearing tokens',
        );
  ...
  Future<dynamic> googleAuth(Map<String, dynamic> authData) async {
    try {
      ...
      if (authData.containsKey('first_name') &&
      ...
      // Restaurant fields
      if (authData.containsKey('restaurant_name') &&
          authData['restaurant_name'] != null) {
        requestBody['restaurant_name'] = authData['restaurant_name']!
            .toString();
      }
      if (authData.containsKey('business_license') &&
          authData['business_license'] != null) {
        requestBody['business_license'] = authData['business_license']!
            .toString();
      }
      ...
      // Driver fields
      if (authData.containsKey('license_number') &&
          authData['license_number'] != null) {
        requestBody['license_number'] = authData['license_number']!.toString();
      }
      if (authData.containsKey('vehicle_type') &&
          authData['vehicle_type'] != null) {
      ...
      return await _handlePublicResponse(
        response,
        endpoint: endpoint,
  ...
  Future<dynamic> _handlePublicResponse(
    http.Response response, {
    String? endpoint,
    String? requestBody,
  }) async {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      try {
        final decodedResponse = json.decode(response.body);
        _logDebug(
          'Public API Success Response Type: ${decodedResponse.runtimeType}',
        );
        return decodedResponse;
      } catch (e) {
